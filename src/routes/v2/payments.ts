...
import { getAttendee, updateAttendee } from "../../services/Attendee/AttendeeService";
...
paymentRouter.get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const priceId = await getEventPriceId(eventId, !!isMember);
      if (priceId) {
        const session = await getOrCreateRSVPCheckoutSession(
          attendee.attendee_id,
          eventId,
          userId,
          priceId
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      return res
        .status(200)
        .json({
          message: `Deleted checkout session for user ${userId} and event ${eventId}`,
        });
    } catch (error) {
      return res.status(500).json(error);
    }
