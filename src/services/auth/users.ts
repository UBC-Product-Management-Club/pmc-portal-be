...
import { formatCSV, fromTable, TABLES } from "./utils";
...
export const getAllSupabaseUsers = async (): Promise<UserRequiredFields[]> => {
    try {
        const { data, error } = await fromTable(TABLES.USER).select();
        if (error || !data) {
...
export const getSupabaseUserByID = async (userID: string): Promise<UserRequiredFields | undefined> => {
    try {
        const { data, error } = await fromTable(TABLES.USER).select().eq("user_id", userID).maybeSingle();
