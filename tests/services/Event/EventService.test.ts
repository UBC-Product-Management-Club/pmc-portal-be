...
describe("EventService", () => {
    ...
    describe('getEvents', () => {
        ...
        it('returns event list successfully', async () => {
            ...
            (EventRepository.getEvents as jest.Mock).mockResolvedValueOnce({ data: testEvents })
        ...
        it('throws error when supabase returns an error', async () => {
            (EventRepository.getEvents as jest.Mock).mockResolvedValueOnce({ error: { message: 'DB Error'}})
    ...
    describe('getEvent', () => {
        ...
        it('returns event', async () => {
            (EventRepository.getEvent as jest.Mock).mockResolvedValueOnce({ data: rawEvent })
            const result = await getEvent('1');
            expect(result).toEqual(parsedEvent);
        });

        it('throws an error when fetch fails', async () => {
            (EventRepository.getEvent as jest.Mock).mockResolvedValueOnce({ error: { message: "Not found" } })
            await expect(getEvent('bad-id')).rejects.toThrow('Not found');
        });

        it('returns null when no event found', async () => {
            (EventRepository.getEvent as jest.Mock).mockResolvedValueOnce({ data: null })
    ...
    describe('getRegisteredEvents', () => {
      ...
      it('returns user events successfully', async () => {
        ...
        (EventRepository.getRegisteredEvents as jest.Mock).mockResolvedValueOnce({ data: mockEvents })
        const result = await getRegisteredEvents(userId);
        expect(result).toEqual(expectedEvents);
      });

      it('returns no events', async () => {
        (EventRepository.getRegisteredEvents as jest.Mock).mockResolvedValueOnce({ data: [] })
